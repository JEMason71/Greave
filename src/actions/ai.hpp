...
private:
    ...
    static const uint32_t   STANCE_AGGRESSIVE_HP_PERCENT;   // When a Mobile's target drops below this many hit points, they'll got to an aggressive stance.
    ...
    static const uint32_t   STANCE_COUNTER_CHANCE;          // 1 in X chance to attempt to counter the target's choice of combat stance.
    static const uint32_t   STANCE_DEFENSIVE_HP_PERCENT;    // Mobiles will switch to defensive stance when their hit points drop below this percentage of maximum.
    ...
    static const uint32_t   STANCE_RANDOM_CHANCE;           // 1 in X chance to pick a random stance, rather than making a strategic decision.
