...
{
public:
    enum class Colour : uint8_t { BLACK, BLACK_BOLD, RED, RED_BOLD, GREEN, GREEN_BOLD, YELLOW, YELLOW_BOLD, BLUE, BLUE_BOLD, MAGENTA, MAGENTA_BOLD, CYAN, CYAN_BOLD, WHITE, WHITE_BOLD, WHITE_BG };
    enum Key { BACKSPACE = 8, TAB = 9, LF = 10, CR = 13, CLOSE = 256, RESIZED, ARROW_UP, ARROW_DOWN, ARROW_LEFT, ARROW_RIGHT, HOME, END, PAGE_UP, PAGE_DOWN, MOUSE_SCROLL_UP, MOUSE_SCROLL_DOWN, MOUSE_LEFT, MOUSE_LEFT_RELEASED,
        MOUSE_MOVED };

    virtual             ~Terminal() { }     // Virtual destructor, should clean up any terminal emulator-specific memory/states.
    virtual int         cell_height() = 0;  // Returns the height of a single cell, in pixels.
    ...
    virtual int         get_mouse_x() = 0;          // Gets the X coordinate for the cell the mouse is pointing at.
    virtual int         get_mouse_x_pixel() = 0;    // Gets the X coordinate for the pixel the mouse is pointing at.
    virtual int         get_mouse_y() = 0;          // Gets the Y coordinate for the cell the mouse is pointing at.
    virtual int         get_mouse_y_pixel() = 0;    // Gets the Y coordinate for the pixel the mouse is pointing at.
    virtual void        get_size(int *w, int *h) = 0;   // Retrieves the size of the terminal (in cells, not pixels).
    ...
    virtual bool        wants_to_close() = 0;   // Returns true if the player has tried to close the terminal window.
};
