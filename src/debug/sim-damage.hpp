...
public:
    ...
    void    sim_armour(uint8_t armour_percent, uint8_t armour_roll_dice, uint8_t armour_roll_die_faces, int8_t armour_roll_modifier = 0);  // Armour damage simulation.

private:
    // Single damage simulation of one weapon type.
    void    sim_armour_individual(const std::string &weapon_name, uint8_t weapon_roll_dice, uint8_t weapon_roll_die_faces, int8_t weapon_roll_modifier = 0);
    ...
    int8_t m_armour_roll_modifier;      // The +/- modifier (if any) to damage reduction on simulated armour.
