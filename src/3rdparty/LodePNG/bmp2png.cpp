...
// Modified by Raine "Gravecat" Simmons, 2016-2021.
...
{
	const std::string source = filename + ".tmp";
	const std::string dest = filename + ".png";
	...
	if (error) throw std::runtime_error("Error decoding bitmap file.");
	...
	if (error) throw std::runtime_error("Error encoding PNG file.");
	temp.swap(png);
	lodepng::save_file(png, dest.c_str());
	error = remove(source.c_str());
	if (error) throw std::runtime_error("Could not delete file: " + source);
	return 0;
}
